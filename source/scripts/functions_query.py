...
PROCEDURES['INSERT_IN_LOCALIZACAO'] = (
    'delimiter //',
...
PROCEDURES['INSERT_IN_CARDAPIO_PASTEL'] = (
    'delimiter //',
...
PROCEDURES['INSERT_IN_FORMA_PAGAMENTO'] = (
    'delimiter //',
...
INSERTS['INSERT_IN_CARDAPIO_PASTEL'] = (
...
INSERTS['INSERT_IN_FORMA_PAGAMENTO'] = (
    "CALL INSERT_IN_FORMA_PAGAMENTO('Dinheiro');",
    "CALL INSERT_IN_FORMA_PAGAMENTO('Cartão');",
)

def query_create_database(cursor) :
    try:
        cursor.execute(
            "CREATE SCHEMA IF NOT EXISTS {} DEFAULT CHARACTER SET utf8 ;".format(DB_NAME))
...
def query_check_dbname(cursor): 
    try:
        cursor.execute("USE {}".format(DB_NAME))
    except mysql.connector.Error as err:
        print("Database {} does not exists.".format(DB_NAME))
        if err.errno == errorcode.ER_BAD_DB_ERROR:
            query_create_database(cursor)
